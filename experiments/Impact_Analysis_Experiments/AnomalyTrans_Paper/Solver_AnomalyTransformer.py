...
from AnomalyTransformer_Model import AnomalyTransformer
...
def solver(cfg, train_loader, test_loader, seed, temperature_value):
    ...
    model = AnomalyTransformer(win_size=window_length, enc_in=input_size, c_out=input_size, e_layers=3)
    ...
    def vali(vali_loader):
        ...
        loss_1 = []
        loss_2 = []
        for batch, inputs in enumerate(vali_loader):
            inputs = inputs[0].to(cfg.device)
            output, series, prior, _ = model(inputs)
            series_loss = 0.0
            prior_loss = 0.0
            ...
            rec_loss = criterion(output, inputs)
    ...
    early_stopping = EarlyStopping(patience=7, verbose=True, dataset_name='selected_dataset')
    ...
    pred_threshold_active = pred
    gt = test_labels.astype(int)
    
    print("pred:   ", pred.shape)
    print("gt:     ", gt.shape)
    
    np.save(f'AnomalyScores_AnomalyTransformer_seed{seed}_dataset_{data_name}.npy', test_energy)
    np.save(f'labels_AnomalyTransformer_seed{seed}_dataset_{data_name}.npy', gt)   
