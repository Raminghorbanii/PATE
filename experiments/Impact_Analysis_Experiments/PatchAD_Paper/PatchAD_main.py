...
from Utils_personal import get_data, generate_loaders, set_seed, load_config
from Solver_PatchAD import solver
...
for seed in seeds:
    seed_results = solver(cfg, train_loader, test_loader, seed, temperature_value = 1)
    results[seed] = seed_results


# Save the results
# with open(f'{cfg.dataset.save_prefix}/results_{cfg.dataset.name}_PatchAD_PaperResults.pkl', 'wb') as f:
